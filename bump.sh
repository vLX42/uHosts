...
sed -i '' \
    -e "s|v${CURRENT_VERSION}/uHosts-${CURRENT_VERSION}.zip|v${NEW_VERSION}/uHosts-${NEW_VERSION}.zip|g" \
    -e "s|>v${CURRENT_VERSION}<|>v${NEW_VERSION}<|g" \
    website/index.html
echo "  ✔ website/index.html"

# README.md — the release.sh comment
sed -i '' "s|dist/uHosts-${CURRENT_VERSION}.zip|dist/uHosts-${NEW_VERSION}.zip|g" README.md
echo "  ✔ README.md"
